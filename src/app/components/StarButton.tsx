...
function StarBackground({ color }: StarBackgroundProps) {
  return (
    ...
    >
      <g clipPath="url(#clip0_star_button)">
        <path
...
type StarButtonProps =
  | (StarButtonBaseProps & React.ButtonHTMLAttributes<HTMLButtonElement> & { href?: undefined })
  | (StarButtonBaseProps & React.AnchorHTMLAttributes<HTMLAnchorElement> & { href: string });
...
export function StarButton({
  ...
  const sharedProps = {
    ...
    className: `star-button group/star-button relative z-[3] inline-flex h-12 items-center justify-center gap-2 overflow-hidden whitespace-nowrap rounded-3xl px-6 py-3 text-sm font-semibold transition-colors disabled:pointer-events-none disabled:opacity-50 ${className}`
  };

  const content = (
    <>
      <div
        className="absolute inset-0 aspect-square bg-[radial-gradient(ellipse_at_center,var(--light-color),transparent,transparent)] animate-[star-button-move_var(--duration)_linear_infinite]"
        style={
          {
            offsetPath: 'var(--path)',
            offsetDistance: '0%',
      ...
      <span className="relative z-10 inline-flex items-center gap-2 text-brand-navy">{children}</span>

      <style>{`
        @keyframes star-button-move {
          to {
            offset-distance: 100%;
          }
        }
